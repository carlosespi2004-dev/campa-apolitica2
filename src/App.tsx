...
type Votante = {
  ...
  barrio: string;
  direccion: string;
  estado: "apoya" | "indeciso" | "no_apoya";
  observacion: string;
  created_at: string;
};
...
export default function App() {
  ...
  async function guardarVotante(e: React.FormEvent) {
    e.preventDefault();
    setGuardando(true);
      ...
      <div className="grid" style={{marginTop: 20}}>
        <div className="card">
          ...
          <form className="form" onSubmit={guardarVotante}>
            ...
            <select
              value={form.estado}
              onChange={(e) => setForm({ ...form, estado: e.target.value as "apoya" | "indeciso" | "no_apoya" })}
            >
            ...
            />
            <button type="submit">{guardando ? "Guardando..." : "Guardar votante"}</button>
          </form>
        </div>
