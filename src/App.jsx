                    ...
                    {(equipo || []).map((m) => (
                      <tr key={m?.id} style={{ borderBottom: "1px solid #f1f5f9" }}>
                        ...
                        <td style={{ padding: 15, textAlign: "center", display: "flex", gap: 5 }}>
                          ...
                          <button onClick={async () => { if (confirm("¿Seguro que deseas eliminar a este miembro completamente (Equipo, Perfil y Acceso)?")) { let uid = m.user_id; if (!uid) { const { data: p } = await supabase.from("profiles").select("user_id").eq("equipo_id", m.id).maybeSingle(); if (p) { uid = p.user_id; } } if (uid) { await supabase.rpc("eliminar_usuario_auth", { uid_usuario: uid }); await supabase.from("profiles").delete().eq("user_id", uid); } await supabase.from("equipo").delete().eq("id", m.id); cargarDatos(); } }} style={{ padding: "6px 12px", background: "#dc2626", color: "white", border: "none", borderRadius: "8px", fontWeight: "800", fontSize: "10px" }}>BORRAR</button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
