...
// BANDERA CON ESCUDO (image_120f87.png)
const ParaguayFlag = () => (
  <svg width="30" height="20" viewBox="0 0 3 2" style={{ borderRadius: '2px' }}>
    ...
    <text x="1.5" y="1.08" textAnchor="middle" fill="#edcb15" fontSize="0.12" fontWeight="900" fontFamily="Arial">★</text>
  </svg>
);
...
export default function App() {
  ...
  const exportarExcel = async () => {
    ...
    const crearHoja = (nombre, lista) => {
      ...
      rTitulo.getCell(1).font = { color: { argb: 'FFFFFFFF' }, size: 20, bold: true };
  ...
  return (
    <div style={{ background: '#f8fafc', minHeight: '100vh', fontFamily: 'Inter, sans-serif' }}>
      <header style={{ background: 'white', padding: isMobile ? '25px 15px' : '40px', textAlign: 'center', boxShadow: '0 4px 15px rgba(0,0,0,0.05)', position: 'relative' }}>
        ...
        <ANRLogo />
        <h1 style={{ fontFamily: 'Montserrat', fontWeight: '900', color: '#C8102E', fontSize: isMobile ? '24px' : '44px', margin: '10px 0 0 0', textTransform: 'uppercase', letterSpacing: '-1.5px' }}>HAGAMOS QUE SUCEDA</h1>
        <div style={{ background: '#f1f5f9', padding: '6px 20px', borderRadius: '50px', display: 'inline-flex', alignItems: 'center', gap: 8, marginTop: 10 }}>
          <ParaguayFlag />
      ...
      <main style={{ maxWidth: '1100px', margin: '0 auto', padding: '30px 15px', paddingBottom: 120 }}>
            ...
            <div className="card" style={{ background: 'white', padding: isMobile ? 25 : 40, borderRadius: '25px', boxShadow: '0 10px 30px rgba(0,0,0,0.03)' }}>
              ...
              <form onSubmit={guardarVotante} style={{ display: 'grid', gap: 15 }}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 15 }}>
                  <div><label style={{fontWeight:'800', fontSize:'11px', color:'#C8102E'}}>NOMBRE</label><input type="text" value={formVotante.nombre} onChange={e => setFormVotante({...formVotante, nombre: e.target.value})} required style={{width:'100%', padding:'14px', borderRadius:'12px', border:'1px solid #e2e8f0'}} /></div>
                  <div><label style={{fontWeight:'800', fontSize:'11px', color:'#C8102E'}}>APELLIDO</label><input type="text" value={formVotante.apellido} onChange={e => setFormVotante({...formVotante, apellido: e.target.value})} required style={{width:'100%', padding:'14px', borderRadius:'12px', border:'1px solid #e2e8f0'}} /></div>
                </div>
                <div><label style={{fontWeight:'800', fontSize:'11px', color:'#C8102E'}}>CÉDULA</label><input type="text" value={formVotante.cedula} onChange={e => setFormVotante({...formVotante, cedula: e.target.value})} required style={{width:'100%', padding:'14px', borderRadius:'12px', border:'1px solid #e2e8f0'}} /></div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 15 }}>
                  <div><label style={{fontWeight:'800', fontSize:'11px', color:'#C8102E'}}>MESA</label><input type="text" value={formVotante.mesa} onChange={e => setFormVotante({...formVotante, mesa: e.target.value})} style={{width:'100%', padding:'14px', borderRadius:'12px', border:'1px solid #e2e8f0'}} /></div>
                  <div><label style={{fontWeight:'800', fontSize:'11px', color:'#C8102E'}}>ORDEN</label><input type="text" value={formVotante.orden} onChange={e => setFormVotante({...formVotante, orden: e.target.value})} style={{width:'100%', padding:'14px', borderRadius:'12px', border:'1px solid #e2e8f0'}} /></div>
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 15 }}>
                   <div><label style={{fontWeight:'800', fontSize:'11px', color:'#C8102E'}}>SECCIONAL</label><input type="text" value={formVotante.seccional} onChange={e => setFormVotante({...formVotante, seccional: e.target.value})} style={{width:'100%', padding:'14px', borderRadius:'12px', border:'1px solid #e2e8f0'}} /></div>
                   <div><label style={{fontWeight:'800', fontSize:'11px', color:'#C8102E'}}>LOCAL</label><input type="text" value={formVotante.local_votacion} onChange={e => setFormVotante({...formVotante, local_votacion: e.target.value})} style={{width:'100%', padding:'14px', borderRadius:'12px', border:'1px solid #e2e8f0'}} /></div>
                </div>
                <div><label style={{fontWeight:'800', fontSize:'11px', color:'#C8102E'}}>BARRIO</label><select value={formVotante.barrio} onChange={e => setFormVotante({...formVotante, barrio: e.target.value})} required style={{width:'100%', padding:'14px', borderRadius:'12px', border:'1px solid #e2e8f0', background:'white'}}><option value="">Elegir barrio...</option>{LISTA_BARRIOS.map(b => <option key={b} value={b}>{b}</option>)}</select></div>
                <div><label style={{fontWeight:'800', fontSize:'11px', color:'#C8102E'}}>RESPONSABLE</label><select value={formVotante.por_parte_de_id} onChange={e => setFormVotante({...formVotante, por_parte_de_id: e.target.value})} required style={{width:'100%', padding:'14px', borderRadius:'12px', border:'1px solid #e2e8f0', background:'white'}}><option value="">¿Quién lo captó?</option>{equipo.map(m => <option key={m.id} value={m.id}>{m.nombre}</option>)}</select></div>
                  ...
                  {(votantes || []).filter(v => (v?.nombre + v?.apellido + v?.cedula).toLowerCase().includes(busquedaLista.toLowerCase())).map(v => (
                    <tr key={v?.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                      ...
                      <td style={{ padding: 15, textAlign:'center', display: 'flex', gap: 5, justifyContent: 'center' }}>
                        <button onClick={() => { setFormVotante(v); setEditIdVotante(v.id); setActiveTab('inicio'); }} style={{ padding: '8px 15px', background: '#f1f5f9', border: 'none', borderRadius: '10px', fontWeight: '800', color: '#64748b', fontSize:'10px' }}>EDITAR</button>
        ...
        {activeTab === 'equipo' && (
          <div style={{ display: 'grid', gap: 30 }}>
            ...
            <div className="card" style={{ background: 'white', padding: 25, borderRadius: '25px' }}>
              <h4 style={{ fontWeight: '900', color: '#C8102E', marginBottom: 20, textTransform: 'uppercase' }}>Miembros Activos</h4>
              <div style={{ overflowX: 'auto' }}>
                    ...
                    {(equipo || []).map(m => (
                      <tr key={m?.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                        ...
                        <td style={{ padding: 15, textAlign: 'center', display: 'flex', gap: 5 }}>
                          ...
                          <button onClick={async () => { if(confirm("¿Eliminar?")) { await supabase.from("equipo").delete().eq("id", m.id); cargarDatos(); } }} style={{ padding: '6px 12px', background: '#dc2626', color: 'white', border: 'none', borderRadius: '8px', fontWeight: '800', fontSize: '10px' }}>BORRAR</button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
