...
// Logo del Candidato Darío Carmona (Incrustado Base64 para estabilidad)
const CarmonaLogo = () => {
  // Cadena Base64 de la imagen subida (logocarmona.png)
...
export default function App() {
  ...
  const exportarExcel = async () => {
    ...
    const crearHoja = (nombreHoja, lista) => {
      ...
      sheet.getRow(2).getCell(1).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFFEE2E2' } };
      sheet.getRow(2).getCell(1).font = { color: { argb: 'FF000000' }, size: 12, bold: true };
  ...
  return (
    <div style={{ background: '#f8fafc', minHeight: '100vh', fontFamily: 'Inter, sans-serif' }}>
      {/* CABECERA REDISEÑADA EXACTAMENTE IGUAL A LA IMAGEN image_67764b.png */}
      <header style={{ background: 'white', padding: isMobile ? '20px 10px' : '40px 20px', textAlign: 'center', boxShadow: '0 4px 15px rgba(0,0,0,0.05)', position: 'relative' }}>
        ...
        {/* FILA DE TEXTOS LATERALES Y LOGO ANR (Alineación perfecta) */}
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: isMobile ? '15px' : '40px', marginBottom: '10px' }}>
          <span style={{ color: '#C8102E', fontSize: isMobile ? '24px' : '48px', fontWeight: '900', fontFamily: 'Montserrat', textTransform: 'uppercase', letterSpacing: '-1.5px' }}>LISTA 2</span>
          <ANRLogo />
          <span style={{ color: '#C8102E', fontSize: isMobile ? '24px' : '48px', fontWeight: '900', fontFamily: 'Montserrat', textTransform: 'uppercase', letterSpacing: '-1.5px' }}>OPCIÓN 5</span>
        </div>

        {/* TÍTULO PRINCIPAL GRANDE Y ROJO */}
        <h1 style={{ fontFamily: 'Montserrat', fontWeight: '900', color: '#C8102E', fontSize: isMobile ? '32px' : '64px', margin: '0', textTransform: 'uppercase', letterSpacing: '-2px' }}>HAGAMOS QUE SUCEDA</h1>
        
        {/* SUBTÍTULO EN PÍLDORA ROJA CON LOGO Y TEXTO BLANCO */}
      ...
      {/* TABS (Resto de la aplicación inalterada) */}
          ...
          <div style={{ display: 'grid', gap: 25 }}>
            ...
            <div className="card" style={{ background: 'white', padding: 30, borderRadius: '25px', boxShadow: '0 10px 30px rgba(0,0,0,0.03)' }}>
              ...
              <form onSubmit={guardarVotante} style={{ display: 'grid', gap: 15 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 15 }}>
                   <input type="text" placeholder="MESA" value={formVotante.mesa} onChange={e => setFormVotante({...formVotante, mesa: e.target.value})} style={{width:'100%', padding:'14px', borderRadius:'12px', border:'1px solid #e2e8f0', fontSize:'16px'}} />
                   <input type="text" placeholder="ORDEN" value={formVotante.orden} onChange={e => setFormVotante({...formVotante, orden: e.target.value})} style={{width:'100%', padding:'14px', borderRadius:'12px', border:'1px solid #e2e8f0', fontSize:'16px'}} />
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 15 }}>
                    ...
                    {(votantes || []).filter(v => (v?.nombre + v?.apellido + v?.cedula).toLowerCase().includes(busquedaLista.toLowerCase())).map(v => (
                      <tr key={v?.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                        ...
                        <td style={{ padding: 15, textAlign:'center' }}>
                          <button onClick={() => { setFormVotante(v); setEditIdVotante(v.id); setActiveTab('inicio'); window.scrollTo(0,0); }} style={{ padding: '8px 12px', background: '#f1f5f9', border: 'none', borderRadius: '10px', fontWeight: '800' }}>EDITAR</button>
            ...
            <div className="card" style={{ background: 'white', padding: 25, borderRadius: '25px' }}>
              {(equipo || []).map(m => (
                <div key={m?.id} style={{ padding: '15px 0', borderBottom: '1px solid #f1f5f9', display:'flex', justifyContent:'space-between' }}>
                  ...
                  <button onClick={() => { setFormEquipo(m); setEditIdEquipo(m.id); window.scrollTo(0,0); }} style={{ padding: '8px 15px', background: '#f1f5f9', border: 'none', borderRadius: '8px', fontWeight: '800' }}>EDITAR</button>
                </div>
              ))}
            </div>
          </div>
