...
export default function App() {
  ...
  // CORRECCIÓN 3: Se cambia por_parte_de_id a equipo_id
  const [formVotante, setFormVotante] = useState({ nombre: "", apellido: "", cedula: "", orden: "", mesa: "", local_votacion: "", seccional: "", barrio: "", equipo_id: "", fecha_nacimiento: "", telefono: "" });
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data }) => {
      ...
      // CORRECCIÓN 1: Pasar el ID del usuario, no el email
  ...
  // CORRECCIÓN 1: Buscar en la tabla profiles por user_id
  async function fetchUserRole(userId) {
    try {
      ...
      if (data) {
        ...
        setCurrentUserId(data.equipo_id);
      } else {
  ...
  async function cargarDatos() {
    ...
    try {
      ...
      // Con tu nuevo RLS, Supabase ya devuelve solo los votantes permitidos para este usuario
      setVotantes(v.data || []);
      setEquipo(e.data || []);
    } catch (err) {
      console.error(err);
  ...
  async function guardarVotante(e) {
    e.preventDefault();
    if (!formVotante.equipo_id) return alert("Selecciona un responsable.");
    ...
    const resp = equipo.find((m) => m.id === formVotante.equipo_id);
    const payload = {
      ...formVotante,
    ...
    if (!error) {
      setFormVotante({ nombre: "", apellido: "", cedula: "", orden: "", mesa: "", local_votacion: "", seccional: "", barrio: "", equipo_id: "", fecha_nacimiento: "", telefono: "" });
      ...
      <main style={{ maxWidth: "1100px", margin: "0 auto", padding: "30px 15px", paddingBottom: 120 }}>
        {activeTab === "inicio" && (
          <div style={{ display: "grid", gap: 25 }}>
            {/* CORRECCIÓN 2: Ocultar padrón si no es administrador */}
            {userRole === "administrador" && (
            ...
            <div className="card" style={{ background: "white", padding: isMobile ? 25 : 40, borderRadius: "25px", boxShadow: "0 10px 30px rgba(0,0,0,0.03)" }}>
              ...
              <form onSubmit={guardarVotante} style={{ display: "grid", gap: 15 }}>
                ...
                {/* CORRECCIÓN 3: Se cambia por_parte_de_id a equipo_id */}
                <div><label style={{ fontWeight: "800", fontSize: "11px", color: "#C8102E" }}>RESPONSABLE</label><select value={formVotante.equipo_id} onChange={(e) => setFormVotante({ ...formVotante, equipo_id: e.target.value })} required style={{ width: "100%", padding: "14px", borderRadius: "12px", border: "1px solid #e2e8f0", fontSize: "16px", background: "white" }}><option value="">¿Quién lo captó?</option>{equipo.map((m) => <option key={m.id} value={m.id}>{m.nombre}</option>)}</select></div>
              ...
              <div style={{ minWidth: isMobile ? "500px" : "100%", overflowY: "auto", maxHeight: "60vh" }}>
                <table style={{ width: "100%", borderCollapse: "collapse" }}>
                  ...
                  <tbody>
                    {(votantes || [])
                      /* CORRECCIÓN 3: Se cambia por_parte_de_id a equipo_id */
                      .filter((v) => userRole === "administrador" || v.equipo_id === currentUserId)
