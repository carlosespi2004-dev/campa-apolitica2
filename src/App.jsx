...
export default function App() {
  ...
  // CORRECCIÓN CONTADOR: Nuevo estado para almacenar el total global saltando el RLS
  const [totalGlobalRPC, setTotalGlobalRPC] = useState(0);
  ...
  const limpiarEstado = () => {
    setVotantes([]); setEquipo([]); setUserRole(null); setUserName(""); setUserEquipoId(null); setTotalGlobalRPC(0);
    setActiveTab("inicio");
  ...
  async function cargarDatos() {
    try {
      ...
      // CORRECCIÓN CONTADOR: Llamada al RPC para obtener el total global exacto saltando el RLS
      const { data: countData } = await supabase.rpc("obtener_total_votantes");
      if (countData !== null && countData !== undefined) {
        setTotalGlobalRPC(countData);
      }
    } catch (err) { console.error(err); }
  }
  ...
  // CORRECCIÓN CONTADOR: Calcula el mayor entre el RLS local y el RPC global
  const totalVotantesGeneral = useMemo(() => {
    const seen = new Set();
    const conteoLocal = votantes.filter(v => { const duplicate = seen.has(normalizarCedula(v.cedula)); seen.add(normalizarCedula(v.cedula)); return !duplicate; }).length;
    return Math.max(conteoLocal, totalGlobalRPC);
  }, [votantes, totalGlobalRPC]); 
